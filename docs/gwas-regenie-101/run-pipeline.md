...
```
nextflow run genepi/gwas-regenie -c first-gwas.config -r v0.3.1 -profile docker -bg
```
In more detail:
...
* `-r v0.3.1` tells which version of the pipeline should be used (you can check the number of the latest version on the GitHub repository of the pipeline and change it accordingly)
