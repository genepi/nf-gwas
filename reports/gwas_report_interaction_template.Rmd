...
for (test in 1:length(split_annotatedTopHits)) {
  ...
  topHitsGroupedByGene <-
    ...
    ungroup() %>%
    select(CHROM, GENPOS, ID, ALLELE0, ALLELE1, GENE_NAME, A1FREQ, LOG10P, BETA, SE ) %>%
    unite("Variant", CHROM:GENPOS, sep=":")  %>%
    unite("Allele 0 / 1", ALLELE0:ALLELE1, sep="/")  %>%
    arrange(desc(LOG10P)) %>%
    rename('Nearest Gene' = GENE_NAME, "Allele 1 Freq" = A1FREQ, 'Effect Size (se)' = SE, 'Beta' = BETA, 'log10(p)' = LOG10P)
