...
regenieTablet <- regenieTable[is.na(regenieTable$Mask) == FALSE, ]
...
cat('**QQ Plot**')
cat('\n\n')
qq_prep <- qqPlotPrepare(pvalues=regenieTable_burdenJoint$LOG10P,ismlog10 = T)
qqPlotFast(qq_prep, ylim = limit)
...
for (test in unique(regenieTable$TEST)){
  ...
  for (settings in unique(regenieTable_tmp01$Settings)) {
    ...
    cat('**QQ Plot**')
    cat('\n\n')
    qq_prep <- qqPlotPrepare(pvalues=regenieTable_tmp02$LOG10P,ismlog10 = T)
    qqPlotFast(qq_prep, ylim = limit)
  }
}
```
