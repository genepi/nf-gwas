...
regenieTable_annotated <- left_join(regenieTable, annotatedTopHits[, c("ID",
                                                                       "GENE_NAME")],
                                    by = "ID")
...
p <- ggplot(don, aes(x=GENPOScum, y=LOG10P)) +
  ...
  ggrepel::geom_text_repel(data = . %>% group_by(GENE_NAME) %>%
                             top_n(1, LOG10P) %>%
                            slice(1) %>%
...
# custom y axis step 2 (depends if ymax is defined)
 ...
 if(params$manhattan_plot_ylimit == 0) {
  ...
  p + scale_y_continuous(expand = c(0, 0),
                       name=expression(-log[10](italic(P))),
                       limits = c(0, params$manhattan_plot_ylimit))
}

```

The red dashed line indicates genome wide significance (*P*=5e-8).  
The gray dashed line indicates suggestive significance (*P*=1e-5).  

  In total, `r nrow(regenieTable[regenieTable$LOG10P > -log10(5e-8),])` SNPs are genome-wide significant.
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
TopHits <- fread(params$annotated_tophits_filename, select = c("ID",
                                                               "ALLELE0",
                                                               "ALLELE1",
                                                               "A1FREQ",
                                                               "BETA",
